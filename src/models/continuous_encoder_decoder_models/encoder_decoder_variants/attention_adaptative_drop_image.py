...
class AdaptativeAttention(nn.Module):
    ...
    def forward(self, encoder_out, decoder_hidden, st):
        ...
        s_att = self.linear_att_s(self.tanh(s + hidden.unsqueeze(1))).squeeze(2)  # right part of eq12
...
class ContinuousDecoderWithAdaptativeAttention(DecoderWithAttention):
    ...
    def init_hidden_state(self, encoder_out):
        ...
        global_image = self.relu(self.linear_image(mean_encoder_out)
                                 )  # (batch_size, 1, decoder_dim)
        V_spatial_features = self.relu(self.linear_regions(encoder_out)
...
class ContinuousAdaptativeAttentionDropImageModel(ContinuousAttentionModel):
