...
class FeaturesAndAttrAttention(nn.Module):
    ...
    def forward(self, encoder_features, encoder_attr, decoder_hidden):
        ...
        batch_attr_embedding = self.embedding_attr.repeat(encoder_features.size()[0], 1, 1)

        attr_embedding_with_attr_score = batch_attr_embedding * encoder_attr.unsqueeze(2)

        att1 = self.encoder_att(attr_embedding_with_attr_score)

        att2 = self.decoder_att(decoder_hidden)  # (batch_size, attention_dim)

        # (batch_size, num_pixels,1) -> com squeeze(2) fica (batch_size, l_regions)
        att = self.full_att(self.relu(att1 + att2.unsqueeze(1))).squeeze(2)
        alpha = self.softmax(att)  # (batch_size, n_attr,1)

        attention_weighted_encoding = (
            self.embedding_attr * alpha.unsqueeze(2)).sum(dim=1)  # (batch_size, encoder_dim)
...
class ContinuousAttentionAttrEmbeddingScoreBeforeImageModel(ContinuousAttentionImageModel):
