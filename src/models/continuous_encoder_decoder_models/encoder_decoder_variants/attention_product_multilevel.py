...
class FeaturesAndAttrAttention(nn.Module):
    ...
    def __init__(self, encoder_dim,  embed_dim, decoder_dim, embedding_attr):
        """
        ...
        self.features_att = nn.Linear(encoder_dim, embed_dim)
        self.decoder_attr_att = nn.Linear(decoder_dim, embed_dim)
        self.decoder_features_att = nn.Linear(decoder_dim, embed_dim)
        self.decoder_attention1_att = nn.Linear(decoder_dim, embed_dim)
        self.decoder_attention2_att = nn.Linear(decoder_dim, embed_dim)
    ...
    def forward(self, encoder_features, encoder_attr, decoder_hidden):
        ...
        scores = torch.matmul(w_h, w_features.transpose(-2, -1)) / math.sqrt(self.dk)
        alpha_regions = self.softmax(scores.squeeze(1))  # (batch_size, l_regions)
        ...
        att_att1 = torch.matmul(w_h, attention1.transpose(-2, -1)) / math.sqrt(self.dk)  # (batch_size, 1)
...
class ContinuousAttrAttentionDecoder(ContinuousDecoderWithAttentionAndImage):
    ...
    def __init__(
        ...
        self.attention = FeaturesAndAttrAttention(
            encoder_dim, embed_dim, decoder_dim, embedding_attr)  # attention network
...
class ContinuousProductAttentionMultilevelAttrEmbeddingAndRegionsImageModel(ContinuousAttentionImageModel):
