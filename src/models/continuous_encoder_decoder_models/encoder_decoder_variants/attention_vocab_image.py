...
class FeaturesAndAttrAttention(nn.Module):
    ...
    def forward(self, encoder_features, encoder_attr, decoder_hidden):
        ...
        attention_weighted_encoding = (self.embedding_attr * alpha).sum(dim=1)  # (batch_size, encoder_dim)
        return attention_weighted_encoding, alpha.squeeze(2)
...
class ContinuousAttentionVocabImageModel(ContinuousAttentionImageModel):
