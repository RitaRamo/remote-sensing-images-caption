...
class ContinuousEncoderDecoderKLCEModel(ContinuousEncoderDecoderModel):
    ...
    def _calculate_loss(self, predict_output, caps, caption_lengths):
        ...
        loss_ce = self.args.w2 * self.criterion_ce(predictions.data, targets.data)

        target_embeddings = pack_padded_sequence(
            target_embeddings, caption_lengths, batch_first=True)

        targets_probs = []
        for target_emb in target_embeddings.data:
            output = torch.cosine_similarity(self.decoder.embedding.weight.data, target_emb.unsqueeze(0))
            target_probs = self.softmax(output)
            targets_probs.append(target_probs.unsqueeze(0))

        targets_probs = torch.cat(targets_probs, dim=0)
        predictions_log = F.log_softmax(predictions.data, -1)
