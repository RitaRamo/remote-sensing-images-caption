...
if __name__ == "__main__":
    ...
    if DATASET == Datasets.RSICD.value:
        torch.save(state, dataset_jsons + "classification_dataset_rsicd_nouns_adjs")
    elif DATASET == Datasets.UCM.value:
        print("len", len(list_wordid))
        torch.save(state, dataset_jsons + "classification_dataset_ucm_nouns_adjs")
    elif DATASET == Datasets.FLICKR8K.value:
        print("len", len(list_wordid))
        torch.save(state, dataset_jsons + "classification_dataset_flickr8k_nouns_adjs")
    else:
